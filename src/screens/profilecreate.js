    ...
    try {
      const response = await fetch(URL.profileUrl, {
        ...
        headers: {
          ...
          Accept: 'application/json, application/xml, multipart/form-data, text/html, text/plain, application/EDI-X12'
        },
        body: JSON.stringify({
          values,
...
return (
  <Keyboardhidecontainer>
    <View style={commonstyles.content}>
      ...
      <View style={{
        alignSelf: 'flex-start', width: '100%', paddingHorizontal: 8,
      }}>
        <SelectorBtn
      ...
      <View style={{ alignSelf: 'flex-start', width: '100%', paddingHorizontal: 8 }}>
        <SelectorBtn label={Language[language]['specialization']}
          name='chevron-down'
          onPress={toggleModal}
          text={selectedSpeciality} />
        <Modal visible={showModal} animationType="slide" transparent={true}>
          <View style={styles.modalContainer}>
            <Text style={{ fontFamily: CUSTOMFONTFAMILY.heading, fontSize: 18, color: CUSTOMCOLOR.black }}>Select Speciality</Text>
            {CONSTANTS.speciality.map((speciality, index) => (
              <Pressable key={index} onPress={() => handleSpecialitySelection(speciality)}>
      ...
      <InputText
        ...
        value={values.experience} setValue={value => handleChangeValue('experience', value)} />
      <Text style={{ fontFamily: CUSTOMFONTFAMILY.h4, fontSize: 12, color: CUSTOMCOLOR.black, alignSelf: 'flex-start' }}>Medical Document</Text>
      <View style={{ alignSelf: 'flex-start' }}><HButton
