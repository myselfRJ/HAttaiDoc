...
import { addpastHistory, updatepastHistory } from '../redux/features/prescription/pastHistory'
...
const PastHistory=()=>{
    ...
    return(
      ...
      {prev?.map((item, ind) => (
        prev.length > 0 ? ( <ShowChip text={item?.past_history} onPress={()=>handleDelete(ind)} ind={ind}/>):null
      ))}
        <PresComponent 
        label='Past History' 
